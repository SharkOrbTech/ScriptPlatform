...
【对白要求】（台词是短剧的灵魂）
- 每集台词量 = 时长秒数 × (3~5)字，第1集最密，后续集可适当灵活
- 台词简短有力，每句只抛一个信息
- 口语化接地气，不秀文笔
...
【台词核心要求】（台词是短剧的灵魂）
- 每集总台词量 = 时长秒数 × (3~5)字（例：90秒→270-450字，60秒→180-300字）
...
3. 台词口语化、有网感，台词量{request.episode_duration * 3}-{request.episode_duration * 5}字（根据{request.episode_duration}秒时长计算）
