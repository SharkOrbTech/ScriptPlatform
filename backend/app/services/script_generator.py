...
【旁白核心要求】（旁白仅限内心独白）
- 旁白仅用于角色内心独白（OS），展示角色内心真实想法，配合表情特写
- 例：「她不知道，这个男人的身份远比她想象的可怕」
- 例：「我发誓，这辈子不会再让任何人欺负我」
- 禁止使用解说旁白和回忆旁白，所有叙事信息必须通过对白传递
- 旁白字数灵活，不设硬性目标，如果旁白在某个镜头中显得生硬，直接省略
- 同一镜头中旁白和台词互斥，只能出现一个
- 台词是叙事的核心驱动力，严禁用旁白代替本该用对话表达的内容
- 绝大多数镜头必须有对话，一个镜头如果没有台词，就没有推进力
...
- video_prompt是给视频生成模型的唯一输入，必须包含该镜头的全部内容：画面+音频+台词+独白
- 必须把该镜头的dialogue和narration原文完整嵌入prompt中，角色说话的内容、语气、情绪都要写清楚
- 必须全部使用中文，禁止出现任何英文单词或短语
- 必须是完整的自然语言段落，像导演在描述一个完整的视频镜头，绝对不能用逗号拼接关键词
- 禁止使用"8K""高清""masterpiece""best quality""wideshot"等参数词
- 所有角色必须用【角色名】格式引用，如【苏婉儿】、【顾北辰】
- 描述必须包含：场景视觉 + 角色动作表情 + 角色说的每句台词（原文原句） + 角色内心独白（原文原句） + 说话语气情绪 + 光影氛围 + 所有音效和环境声
- 如果该镜头有dialogue，必须在prompt中写出【角色名】说"原句"；如果有narration内心独白，必须写出【角色名】内心独白"原句"
- 用户可以直接复制粘贴到视频生成模型中使用

正确示例（有台词）：
"【苏婉儿】站在破旧的出租屋内，昏黄的灯光照亮她苍白憔悴的脸庞，窗外是繁华的城市夜景与屋内的冷清形成鲜明对比。她紧握着手机，手指微微发颤，咬着嘴唇低声说'我回来了，这一次不会再让任何人欺负我'，语气带着压抑的颤抖和决心，眼眶渐渐泛红。远处传来隐约的汽车鸣笛声和雨声簌簌。镜头缓慢推近她的面部，捕捉她眼中闪烁的泪光和逐渐坚定的眼神。"

正确示例（有内心独白，无台词）：
"【顾北辰】独自站在落地窗前，窗外是城市璀璨的夜景，他深邃的侧脸映在玻璃上。他的手指无意识地摩挲着无名指上的银色戒指，眼神幽暗如深渊。他内心独白的声音低沉而压抑：'她不知道，这个男人的身份远比她想象的可怕'。背景安静得只剩下空调的嗡嗡声。镜头从他的背影缓慢环绕至侧面，捕捉他嘴角那一丝苦涩的弧度。"
...
普通冲突、普通对话、情绪过渡、场景切换等日常内容，一律标记为null。
不要在每一个有情绪变化的镜头上标hook_type。
  ...
  "shots": [
    {
      ...
      "duration": 3.0,
      "video_prompt": "视频生成提示词（全中文自然语言段落，完整描述场景视觉+【角色名】动作表情+角色台词原文原句+内心独白原文原句+说话语气+情绪氛围+光影+音效+环境声，音频画面一起生成。禁止英文关键词/8K/masterpiece/高清等术语，禁止逗号拼接关键词列表）",
    ...
    {{
      ...
      "shots": [
        {{
          ...
          "dialogue": "台词（含情绪标注）",
          "duration": 3.0,
          "video_prompt": "视频生成提示词（全中文自然语言，角色用【角色名】引用，包含场景+动作+情绪+光影+音效+台词原文+内心独白原文，音频画面一起生成）",
          "hook_type": "钩子类型",
          "hook_detail": "钩子分析"
        }}
      ]
