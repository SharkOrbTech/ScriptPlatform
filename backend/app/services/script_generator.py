...
【视频生成提示词要求】（关键变化，替代旧的AI绘图提示词！每个镜头必须生成video_prompt）
...
- 描述必须详细包含：场景视觉构成 + 角色动作表情 + 台词原文（如有） + 情绪氛围 + 光影效果 + 背景音效 + 环境声音
- 用户可以直接复制粘贴到视频生成模型中使用
- 长度至少100字，确保足够详细

正确示例：
"【苏婉儿】站在破旧的出租屋内，昏黄的灯光照亮她苍白憔悴的脸庞，窗外是繁华的城市夜景与屋内的冷清形成鲜明对比。她紧握着手机，手指微微发颤，咬着嘴唇低声说道"我回来了，这一次不会再让任何人欺负我"，语气带着压抑的颤抖和决心。远处传来隐约的汽车鸣笛声和雨声簌簌。镜头缓慢推近她的面部，捕捉她眼中闪烁的泪光和逐渐坚定的眼神。"
  ...
  "shots": [
    {
      ...
      "duration": 3.0,
      "video_prompt": "视频生成提示词（全中文自然语言段落，完整描述场景视觉+【角色名】动作+情绪氛围+光影+音效+角色台词，禁止英文关键词/8K/masterpiece/高清等术语，禁止逗号拼接关键词列表）",
    ...
    {{
      ...
      "shots": [
        {{
          ...
          "dialogue": "台词（含情绪标注）",
          "duration": 3.0,
          "video_prompt": "视频生成提示词（全中文自然语言，角色用【角色名】引用，包含场景+动作+情绪+光影+音效+台词）",
          "hook_type": "钩子类型",
          "hook_detail": "钩子分析"
        }}
      ]
...
2. 每个分镜都有完整的video_prompt（视频生成提示词，全中文自然语言，角色用【角色名】引用）
