...
- 单集内部结构：10%-80%-10%黄金比例
  - 起因（10%）：以明确的戏剧张力开场（紧张对峙、关键对话、重要发现），零铺垫
...
【钩子技法体系】
开头钩子类型（必须在3秒内抓住观众）：
A. 戏剧张力开场：直接展示核心矛盾（对峙、决裂、危机时刻）
B. 身份反转开场（仅限第1集）：开场即展示核心矛盾（前世结局→重生→改变命运）。可用连续几个镜头的第一人称OS切入——"我重生了，回到了命运转折的那一天""上一世，我输得一败涂地。这一世，我要他们百倍偿还""睁开眼睛的那一刻，我发现自己回到了三年前"。开场可以用连续几个旁白镜头讲清前世关键事件，之后立刻进入场景
...
- 反派要有具体的恶行细节（具体行为、具体事件），不能抽象概括
- 每个反派都要有具体的恶行细节（具体行为、具体事件），不能抽象概括
...
- 例："我发誓，这辈子不会再让任何人欺负我"
- 重生/穿越题材第1集开场OS（仅限第1集开头，可用连续几个旁白镜头讲清前世）：
  "我重生了，回到了命运被改写的那一天""上一世，我输得一败涂地。这一世，我要他们百倍偿还""睁开眼睛的那一刻，我发现自己回到了三年前"
- 使用规则：
  - 第1集开场例外：允许连续2~4个镜头的第一人称OS，用于快速交代前世背景。之后立刻转入场景对话
  - 其他集数旁白：旁白字数灵活，不设硬性目标，如果生硬就省略。一集1-2处内心独白即可
  - 同一镜头中旁白和台词互斥，只能出现一个
...
【爽感来源】爽感来自身份反差、绝境反杀、以弱胜强。自然融入剧情，不要机械插入
常见形态（供参考，并非都要用）：身份碾压、扮猪吃老虎、重生改变命运、大佬对小人物恭敬、聚会遭歧视后反转打脸、将计就计以牙还牙
...
{
{
  ...
  "episode_plan": [
    {
      "episode_number": 1,
      "title": "集标题（简洁，2-5字，描述本集核心事件，不写成宣传语）",
      "key_conflict": "核心冲突",
      "key_events": ["事件1", "事件2"],
...
- 经过（80%）：通过人物互动和对白推进剧情。情绪自然递进，不必每集都是戏剧循环。这是情绪的蓄水池。
- 结果/钩子（10%）：留悬念设卡点，让观众想看下一集。悬念可以是信息差、人物决定、情感转折
...
【闪回技法】（核心叙事工具）
- 用闪回打断当前时间线来揭示关键信息：前世的记忆、过去的创伤、隐藏的关系
- 闪回触发点：一个物品（戒指、照片）、一句话、一个场景——触发了就切闪回
- 格式：在frame_content中用（闪回）标注开头，用（现实）标注切回
- 闪回不超过3个镜头，内容要短而精，只展示最关键的信息
- 第1集至少用1次闪回揭示核心动机
...
- 对话要有来有回：A说→B反应→B说→A反应，不要一个角色长篇大论
	...
	【对话节奏】（关键场景深挖，次要场景快切）
	- 每集选1-2个关键场景深入展开：A说→B反应→B说→A再回应，来回3-5轮。这是建立人物关系的地方
	- 其余场景可以快切，1-2轮对话就够了。不要每个场景都深挖
	- 对话中穿插动作和反应：角色说话时做什么、对方听完什么表情、谁停顿了——这些比台词本身更有信息量

	【人物语气区分】
	- 每个主要角色要有可辨识的说话方式。不要所有人都用同一种语气
	- 反派不一定都靠吼。可以阴阳怪气、可以表面温柔实则威胁、可以冷暴力
	- 主角的台词不要每句都狠。可以让步、可以示弱、可以沉默——反差才动人
- 场景切换：2-3个镜头构成一个场景段落，切换场景时直接在frame_content中写明新地点
- 内心独白（narration）只用在角色特写时配合表情使用，一集最多1-2处
...
- 禁止回忆旁白，所有背景信息优先通过对白带出。但第1集开场的重生/穿越OS不受此限制
- 第1集开场例外：允许连续2~4个镜头的第一人称OS，用于快速交代前世背景。之后立刻转入场景对话
- 其他集数：一集最多1-2处内心独白，放在情感最关键的时刻。如果旁白在某个镜头中显得生硬，直接省略
- 同一镜头中旁白和台词互斥，只能出现一个
...
【台词写作原则】（参考，根据角色性格灵活创作）
- 每个角色的台词应符合其身份、性格和处境，不要套用模板
- 反派台词要有具体的恶行细节（"你爸妈是我撞死的"），不能抽象概括
- 主角反击台词应简短有力，一个动作或一句话完成反转
- 身份揭露时不要一次性说完，逐步释放信息

示例参考（勿照抄，按角色灵活变通）：
- 身份揭露："谁说XX和XX退婚了？""以后谁与你为敌就是与XX集团为敌"
- 反转打脸："不好意思，这个项目已经被我拿下了""你以为你赢了？好戏才开始"
- 反击："我讲话，你插什么嘴？懂不懂规矩？"

【反派塑造要求】
- 反派的核心魅力：他/她觉得自己是正义的，行为有自洽的逻辑，不是单纯的恶
- 反派得意时主动揭露阴谋是核心爽感来源（"你还不知道吧？你爸妈是我撞死的！"）
- 反派恶行要有具体细节，不抽象概括
- 反派被打脸后可找更强大靠山，形成套娃式升级
- 不要每句都是侮辱。反派的压迫感来自处境，不是音量
- 反派嘲讽要具体："就凭你也配？""你这种人给我提鞋都不配"
- 反派出手要狠（"一脚踹在她脸上"），不要心慈手软

【钩子技法应用】（自然融入，不强制每集全部使用）
开头钩子（前3秒尽量有，但不强求每个镜头都是爆点）：
- 暴力冲突、身份反转、极端情绪、悬念氛围等开场
- 重生/穿越题材（仅限第1集开头）：可用连续几个镜头的第一人称OS切入——"我重生了""上一世……"——连续2~4个旁白镜头讲清前世关键事件，之后立刻进入场景对话。后续集数根据剧情需要自然地使用第一人称OS即可，不必重复"我重生了"这种开场式句式
...
- 【角色名】格式是强制要求：每个出现的人名必须用【】括起来，漏掉任何一个角色名都是错误。正确：【苏婉儿】说："原文"；错误：苏婉儿说："原文"
...
【爽感来源】（适当融入剧情）
爽感来自身份反差、绝境反杀、以弱胜强。自然融入，不要机械插入
常见形态（供参考，并非都要用）：身份碾压、扮猪吃老虎、重生改变命运、大佬对小人物恭敬、聚会遭歧视后反转打脸、将计就计以牙还牙
  ...
  "shots": [
    {
      ...
      "dialogue": "台词（只写角色说的话，情绪标注放在frame_content中）",
      "narration": "旁白/内心独白（仅限内心独白，台词与旁白在同一镜头中互斥，不能同时出现）",
      "bgm_suggestion": "背景音乐建议",
      "duration": 3.0,
      "video_prompt": "视频生成提示词（所有角色名必须用【】括起来。格式：【角色名】做什么动作/说什么话"原文"+语气+音量/内心声音OS"原文"+音色/具体音效/镜头运动。禁止文学修辞，禁止逗号拼接关键词）",
    ...
    {{
      ...
      "shots": [
        {{
          ...
          "dialogue": "台词（含情绪标注）",
          "duration": 3.0,
          "video_prompt": "视频生成提示词（写给AI视频模型的指令，直接描述画面和声音。【角色名】说"原文"+语气/内心声音OS"原文"+音色/具体音效/镜头运动。所有角色名必须用【】括起来）",
          "hook_type": "钩子类型",
          "hook_detail": "钩子分析"
        }}
      ]
...
【热点灵感参考】（重要：仅提取核心创意元素和情感共鸣点，绝对不要直接复制标题或描述，人物名字必须全部原创不得与热点中任何角色同名，必须原创一个全新的故事）
...
1. 故事必须原创，不要套用热点内容，所有人名必须全新创作不得与参考材料中任何角色重名
