...
- **小说智能改编**: 异步上传+进度轮询，分层摘要+关键章节提取，并行episode生成，知识库一致性保障
- **智能章节检测**: 四层递进式检测（DOCX标题→正则显式标记→统计预过滤+LLM确认→等长兜底），不塞全文进上下文，TX T无标记也能分章
...
不依赖单一方法，从最准确到最兜底逐层尝试：

```
┌─────────────────────────────────────────────────┐
│ 第1层: DOCX Heading 样式                         │
│   python-docx 遍历段落树 → Heading 1/2/3 = 章节  │
│   准确度: ★★★★★ (Word 原生结构)                  │
├─────────────────────────────────────────────────┤
│ 第2层: 正则显式标记 (纯文本 .txt)                 │
│   第X章 / Chapter N / 序章 / 楔子 / 卷X / ===    │
│   准确度: ★★★★ (格式规范时)                      │
├─────────────────────────────────────────────────┤
│ 第3层: 统计预过滤 + 1次 LLM 采样确认              │
│   ┌─ 6种无AI统计信号 (O(n)):                    │
│   │   • 显式标记 (第X章/Chapter N/序章/卷X)      │
│   │   • 排版分隔符 (***, ---, ===)               │
│   │   • 短标题候选 (短段落+前后长段落)            │
│   │   • 时间转移 (正则: \d+天后/X个月后...)       │
│   │   • 词汇变化 (相邻段落词语重叠率骤降)          │
...
**关键设计原则**:
- **LLM 绝不见全文**: 统计预处理把候选边界从数千段落过滤到 ≤15 个，LLM 只看每个候选周围 ~300 字
- **即使 TXT 无"第X章"也能分**: 时间转移词（`三天后`、`与此同时`）+ 词汇变化 + 段落长度异常足以定位隐式边界
- **假阳性控制**: 正文中的"第三章"词汇变化得分远低于真正的 `\n第三章` 显式标记（5 vs 1）
- **自适应**: 优先用最准确的方法，不满足才降级
