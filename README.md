...
- **分镜脚本**: 每集精确到每个分镜的景别、运镜、画面、台词、旁白、AI绘图提示词
- **旁白叙事系统**: 旁白作为第一叙事手段（内心独白、解说旁白、回忆旁白），30-50%分镜包含旁白
- **钩子标记系统**: 自动标注爆点、悬念、伏笔、反转等7种钩子类型，前端彩色高亮显示
...
- 压-爽节奏单元
- 旁白叙事要求：30-50%分镜需包含旁白（内心独白/解说旁白/回忆旁白）
- 台词核心要求 + 高频台词模式（台词300-400字，旁白200-300字）
- 钩子类型标记（7种类型 + 详细分析）
- 景别/运镜/转场规范
- AI绘图提示词公式
...
快速E2E测试使用 httpx AsyncClient + ASGITransport 直接测试 FastAPI 应用，无需启动真实服务器，Mock 所有LLM调用，覆盖：登录认证、权限管理、剧本生成全流程（含旁白字段验证）、状态轮询、剧本详情、列表查询、QA问答、小说上传、密码修改、200集上限验证等。
